...
class EditCooler extends Component {
  state = {
    newCooler: {
      name: ""
    },
  ...
  //renders data from database on page load via CoolerSetup saga
  componentDidMount = () => {
  ...
  addCoolerSource = event => {
    ...
    this.props.dispatch({
      type: "ADD_COOLER_SOURCE",
      payload: this.state.newCooler
    });
    this.setState({
      newCooler: {
        name: ""
  ...
  //removes seleted water labels by calling the CoolerSetup saga and then re-rendering the label list
  // if delete is carried out, delete button is then disabled
  removeCoolerSource = () => {
    swal({
      title: `Delete (${this.state.checked.length}) Coolers?`,
      text:
        "These Coolers will be removed from your harvest year but will still appear in your records",
      icon: "warning",
      buttons: true,
      dangerMode: true
    }).then(willDelete => {
  ...
  //handles wheter item is checked or not by passing through the farm_Cooler_id of the item being clicked
  ...
  handleClickOpen = i => {
    this.setState({
      ...this.state,
      dialogState: {
        array: this.props.reduxState.setupFacilities.setupCoolerReducer[i]
      },
      setOpen: true
    });
  };
  ...
  //on clicking the update button, any changes made are sent to database by calling the CoolerSetup saga and window closes
  handleClose = event => {
        ...
        >
          <Grid item xs={12} sm={6}>
            <Typography
              ...
              align="center"
            >
              Add or Edit coolers You Want to Track
            </Typography>
          </Grid>

          <Grid item xs={12} sm={6}>
            <TextField
              label="Coolers to track"
              variant="outlined"
              color="primary"
          ...
          <Grid item xs={12} sm={6}>
            ...
            >
              ...
              />
              <Typography className={classes.fabColor}>Add Cooler</Typography>
            </Button>
          </Grid>
          ...
          <Grid item xs={12} sm={6}>
            <ExpansionPanel style={{ width: "80vw", maxWidth: 300 }}>
              <ExpansionPanelSummary
                ...
                id="panel1a-header"
              >
                <Typography className={classes.heading}>My Coolers</Typography>
              </ExpansionPanelSummary>
              <ExpansionPanelDetails>
                <Grid item xs={12} sm={6}>
                  ...
                  >
                    {this.props.reduxState.setupFacilities.setupCoolerReducer.map(
                      (Cooler, i) => (
                        <section key={Cooler.farm_cooler_id}>
                          <ListItem
                            key={Cooler.farm_cooler_id}
                            style={{
                              display: "flex",
                              direction: "column",
                              width: "70vw",
                            ...
                            onClick={this.handleCheck(Cooler.farm_cooler_id)}
                          >
                            <ListItemIcon>
                              <Checkbox
                                edge="start"
                            ...
                            <ListItemText
                              primary={Cooler.farm_cooler_name}
            ...
            >
              <DialogContent style={{ width: "80vw", maxWidth: 200 }}>
                <TextField
                  ...
                  id="name"
                  label={"Cooler Name"}
                  value={this.state.dialogState.array.farm_cooler_name}
                  onChange={this.handleDialogChangeFor("farm_cooler_name")}
                  fullWidth
                />
              </DialogContent>
              <DialogActions>
...
export default connect(mapReduxStateToProps)(withStyles(styles)(EditCooler));
