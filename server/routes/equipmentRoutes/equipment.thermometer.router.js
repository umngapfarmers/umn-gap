...
router.post('/new', (req, res) => {
  ...
  pool.query(sqlText, values)
    .then((result) => {
      console.log('added farm thermometer ');
      res.sendStatus(201);
    })
    .catch((error) => {
      console.log(`error in farm_thermometer post `, error);
      res.sendStatus(500);
    })
});
...
router.get('/', rejectUnauthenticated, (req, res) => {
  ...
  let sqlQuery = `SELECT * FROM "farm_thermometer" WHERE "harvest_year_id" = $1;`
...
router.delete('/delete/:id', rejectUnauthenticated, (req, res) => {
  const queryText = 'DELETE FROM "farm_thermometer" WHERE "farm_thermometer_id" = $1;';
